...
export class Argument implements IArgument {
  static from(
    child: IArgument['value'] | Partial<IArgument>,
    parent?: Partial<IArgument>
  ): Argument {
    if (!parent) {
    ...
    switch (typeof child) {
      ...
      case 'object': {
        ...
        const { value, rules = parent.rules } = child;
  ...
  static format(template: string, fields: Map<string, string>): string {
    return template.replace(
      /\$\{(.*?)}/g,
      (key) => fields.get(key.substring(2).substring(0, key.length - 3)) ?? key
    );
