...
import { ArtifactDownloadTask } from './artifact-download-task';
import { ArtifactExtractTask } from './artifact-extract-task';
...
export class Artifact implements IArtifact {
  static from(
    child: string | Partial<IArtifact>,
    parent?: string | Partial<IArtifact>
  ): Artifact {
    switch (typeof child) {
      case 'string':
      ...
      case 'object': {
        ...
        const _parent =
          typeof parent !== 'string' ? parent : Artifact.fromId(parent);
        const {
          path = _parent.path,
          url = _parent.url,
          size = _parent.size,
  ...
  static fromId(
    ...
    if (parts.length < 3)
      throw new Error('passed string is not include a valid artifact id');

    const [group, artifact, unsplittedVersion] = parts;
    const [version, versionExtension = defaultExtension] =
      unsplittedVersion.split('@');
    const paths: string[] = [...group.split('.'), artifact, version];

    if (parts.length > 3) {
      const [unsplittedClassifier] = parts.slice(3);
      const [classifier, classifierExtension = defaultExtension] =
        unsplittedClassifier.split('@');
  ...
  toString(defaultExtension = 'jar'): string {
    ...
    if (!target || !version || !artifact)
      throw new Error('artifact path parse error');
    ...
    const ext =
      i >= 1
        ? splittedTarget.slice(i + 1).join(targetSep)
        : splittedTarget.reverse().shift()?.replace(version, '') ||
          defaultExtension;
    ...
    if (ext.startsWith(extSep))
      return (
        [group, artifact, version].join(idSep) + '@' + ext.replace(extSep, '')
      );

    const splittedExtension = ext.split(extSep);
    const classifier = splittedExtension.shift();
    return (
      [group, artifact, version, classifier].join(idSep) +
      '@' +
      splittedExtension.join(extSep)
    );
  }
  ...
  extractTo(
    ...
    return new ArtifactExtractTask(
      this,
      artifactDirectory,
      extractionDirectory,
      exclude
