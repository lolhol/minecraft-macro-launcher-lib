import { Rule, IRule } from '../rule/rule';
import { Platform, OS, IPlatform } from '../platform/platform';
import { ILibraryDownloads, LibraryDownloads } from './library-downloads';
...
export class Library implements ILibrary {
  static from(lib: string | Partial<ILibrary>): Library {
    ...
    const { name, downloads = {}, extract, rules, natives } = lib;
  ...
  isApplicable(
    ...
    return !this.rules
      .map((rule) => rule.isAllowable(platform, features))
      .includes(false);
  }
  ...
  getNativeClassifier(
    ...
    return format
      ? Argument.format(
          classifier,
          new Map([['arch', arch.match(/\d\d/g)?.shift() ?? '32']])
        )
      : classifier;
  }
  ...
  toJSON(): ILibrary {
    const { name, downloads, natives, extract, rules } = this;
    return {
      name,
      downloads,
      natives,
