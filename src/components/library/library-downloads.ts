import { Artifact, IArtifact } from '../artifact/artifact';
...
export class LibraryDownloads implements ILibraryDownloads {
  static from(
    ...
    const { natives = {}, name } = parentLibrary;

    if (!name) throw new Error('missing parent library name');

    const { artifact = {}, classifiers = {} } = libraryDownloads;

    Object.entries(natives)
      .map(([os, classifier]) => {
        if (typeof classifier !== 'string')
          throw new Error('library native artifact classifier is not string');
        return {
          classifier,
          os,
      ...
      .forEach(({ classifier }) => {
        const nameWithNativeClassifier = Library.concatNameWithClassifier(
          name,
          classifier
        );
    ...
    return new LibraryDownloads(
      ...
      Object.fromEntries(
        Object.entries(classifiers).map(([classifier, artifact]) => {
          const nameWithClassifier = Library.concatNameWithClassifier(
            name,
            classifier
          );
  ...
  constructor(
    artifact: Partial<IArtifact>,
    classifiers: ILibraryDownloads['classifiers'] = {}
  ) {
    Object.entries(classifiers).forEach(([classifier, artifact]) =>
      this.setArtifactByClassifier(classifier, artifact)
    );
  ...
  toJSON(): ILibraryDownloads {
    const { artifact, classifiers } = this;
    return {
      artifact,
      classifiers,
    };
