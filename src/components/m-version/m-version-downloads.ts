...
export class MVersionDownloads implements IMVersionDownloads {
  static from(
    child: Partial<IMVersionDownloads>,
    parent?: Partial<IMVersionDownloads>
  ): MVersionDownloads {
    if (!parent) {
    ...
    const { client: parentClient = {}, server: parentServer = {} } = parent;
    const { client = parentClient, server = parentServer } = child;
  ...
  constructor(client: Partial<IArtifact>, server: Partial<IArtifact>) {
