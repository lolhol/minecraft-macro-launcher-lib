import { Argument, IArgument } from '../argument/argument';
...
export class MVersionArguments implements IMVersionArguments {
  ...
  static from(
    child: Partial<IMVersionArguments>,
    parent?: Partial<IMVersionArguments>
  ): MVersionArguments {
    if (!parent) {
    ...
    const { game = parent.game, jvm = parent.jvm } = child;
