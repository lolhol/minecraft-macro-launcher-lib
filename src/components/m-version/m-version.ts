...
import {
  ...
  IMVersionAssetIndexArtifact,
} from './m-version-asset-index-artifact';
import { ILibrary, Library } from '../library/library';
...
export class MVersion {
  static from(
    child: Partial<IMVersion>,
    parent?: Partial<IMVersion>
  ): MVersion {
    if (!parent) {
    ...
    if (!mainClass)
      throw new Error('minecraft version main class is not string');
...

    {
      ...
      _libs
        .filter((_lib) => !flatLibs.includes(_lib.name))
        .forEach((_lib) => libs.push(_lib));
    } // consolidate libs
...

    {
      ...
      {
        const flatGameArgs = versionArgs.game.map(({ value }) =>
      ...
      {
        const flatJvmArgs = versionArgs.jvm.map(({ value }) =>
          value.join(flatArgsSep)
        );
        _versionArgs.jvm
          .filter((_arg) => {
    ...
    if (minecraftArguments) {
      const { game, jvm } =
        MVersionArguments.fromLegacyArguments(minecraftArguments);
