...
export class Asset {
  static from(
    child: Omit<Partial<IAsset>, 'path'>,
    parent?: Partial<IAsset>
  ): Asset {
    if (!parent) {
    ...
    const { hash = parent.hash, size = parent.size } = child;
  ...
  constructor(public path: string, public hash: string, public size: number) {}
