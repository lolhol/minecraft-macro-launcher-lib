...
describe('Asset', () => {
  ...
  describe('#legacyPath', () => {
    it('should return valid legacy path', () => {
      Object.keys(objects).forEach((path) => {
        expect(Asset.from(objects[path], { path }).legacyPath).toBe(
          join('virtual', 'legacy', path)
        );
      });
    });
  });
  ...
  describe('#objectPath', () => {
    it('should return valid object path', () => {
      Object.entries(objects).forEach(([path, { hash }]) => {
        expect(Asset.from(objects[path], { path }).objectPath).toBe(
          join('objects', subhashes[hash], hash)
        );
      });
    });
  });
  ...
  describe('#toArtifact', () => {
    it('should returns new artifact instance', () => {
      Object.entries(objects).forEach(([path, { hash, size }]) => {
        expect(
          new Asset(path, hash, size).toArtifact() instanceof Artifact
        ).toBeTruthy();
      });
    });
  });
